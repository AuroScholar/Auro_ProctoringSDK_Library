...
class NoiseDetector() {
  ...
  private var audioRecord: AudioRecord? = null
    ...
    fun start(context: Context,listener: FaceDetector.OnProctoringResultListener) {
       ...
       /* val buffer = ShortArray(BUFFER_SIZE)
        while (isRunning) {
            ...
            if (readSize != AudioRecord.ERROR_INVALID_OPERATION) {
                ...
                if (amplitude > 1000) {
                    faceMovementDetectionListener.onVoiceDetected(amplitude,true,isRunning)
                }else{
                    faceMovementDetectionListener.onVoiceDetected(amplitude,false,isRunning)
                }
            }
        }*/
